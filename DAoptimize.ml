  ...
  | Some cfgNode ->
      ...
      then (
        ...
        *)
        ...
        let new_in_set =
          StringSet.union !gen_set (StringSet.diff in_set !kill_set)
        in

        (* begin analysis of remaining branches and return logical OR of all *)
        dce cfgNode.true_branch new_in_set
        || dce cfgNode.false_branch new_in_set
        || not (StringSet.is_empty !kill_set))
      else false
